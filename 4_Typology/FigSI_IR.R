...
diadsP <- diads %>%
  mutate(
    ...
    class = case_when(
      ...
      pmax(IR_B2_1, IR_B2_2) < eps &
        pmax(UR_B2_1, UR_B2_2) < eps ~ "IU0",
      
      (IR_B2_1 < eps & UR_B2_1 < eps) |
        (IR_B2_2 < eps & UR_B2_2 < eps) ~ "IU1",
      
      pmin(IR_B2_1, IR_B2_2) < eps ~ "UB",
...
diadsF <- diads %>%
  mutate(
    IR_1 = pmax(IR_1, CR3_1),
    IR_2 = pmax(IR_2, CR3_2),
    IR_B2_1 = pmax(IR_B2_1, CR3_B2_1),
    IR_B2_2 = pmax(IR_B2_2, CR3_B2_2),
    IR_B1_1 = pmax(IR_B1_1, CR3_B1_1),
    IR_B1_2 = pmax(IR_B1_2, CR3_B1_2)
  ) %>%
  mutate(
    rG1_1 = IR_B1_1 / IR_1,
    rG1_2 = IR_B1_2 / IR_2,
...
ggsave("./plts/IR/cls_1.pdf",p1,width=4,height=3)
...
write.csv(data_alluvial,
          "./plts/IR/data_cls_1_alluvial.csv",
          row.names=FALSE)
...
diadsP <- diadsP %>%
  mutate(
    FIR = (IR_1 + IR_2)/(IR_C_1 + IR_C_2),
    FBW = (IR3_1 + IR3_2)/(IR_1 + IR_2),
    IR  = (IR_1 + IR_2)*1000
  )
...
ggsave("./plts/IR/cls_2.pdf",ptuck,width=5,height=3)
