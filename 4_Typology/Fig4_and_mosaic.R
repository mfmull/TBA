...
diadsP <- diads %>%
  mutate(
    ...
    class = case_when(
      ...
      pmax(GW_B2_1, GW_B2_2) < eps &
        pmax(UR_B2_1, UR_B2_2) < eps ~ "IU0",
      
      (GW_B2_1 < eps & UR_B2_1 < eps) |
        (GW_B2_2 < eps & UR_B2_2 < eps) ~ "IU1",
      
      pmin(GW_B2_1, GW_B2_2) < eps ~ "UB",
...
diadsF <- diads %>%
  mutate(
    ...
    GW_B2_1 = pmax(GW_B2_1, CR3_B2_1),
    GW_B2_2 = pmax(GW_B2_2, CR3_B2_2),
...
write.csv(diadsC,"SI2.csv",row.names=FALSE)
...
ggsave("./plts/Fig4/cls_1.pdf",p1,width=4,height=3)
...
write.csv(data_alluvial,
          "./plts/Fig4/data_cls_1_alluvial.csv",
          row.names=FALSE)
...
ggsave("./plts/Fig4/cls_2.pdf",ptuck,width=5,height=3)
...
write.csv(data_tukey,
          "./plts/Fig4/data_cls_2_bars.csv",
          row.names=FALSE)



############################################################
# 9. Mean-normalized groundwater area
############################################################

data_lgw <- diadsP %>%
  filter(GW>0) %>%
  mutate(LGW = GW/mean(GW,na.rm=TRUE)) %>%
  group_by(class) %>%
  summarise(
    mean = mean(LGW),
    se   = sd(LGW)/sqrt(n()),
...
write.csv(data_lgw,
          "./plts/Fig4/data_cls_s1_lgw.csv",
          row.names=FALSE)


ps1 <- ggplot(data_lgw,aes(class,mean,fill=class))+
  geom_col(width=.6)+
  geom_errorbar(aes(ymin=mean-se,ymax=mean+se),width=.1)+
  scale_fill_manual(values=cols)+
  scale_y_log10()+
  ylab("Mean-normalized GW-irrigated cropland")+
  theme_classic()+
  theme(legend.position="none")

ggsave("./plts/Fig4/cls_s1.pdf",ps1,width=2.5,height=3)



############################################################
# 10. Land-use dominance classification
############################################################

diadsL <- diads %>%
  mutate(
    maxD = case_when(
      pmax(Area_B1_1/Area_1,Area_B1_2/Area_2)>=thresh ~ "0-10km",
      pmax(Area_B2_1/Area_1,Area_B2_2/Area_2)>=thresh ~ "10-100km",
      TRUE ~ "100+km"
    ),
    maxD=factor(maxD,levels=c("0-10km","10-100km","100+km"))
  )
...
plot_mosaic <- function(data,var1,var2,file){
  
  tab <- xtabs(as.formula(paste("~",var1,"+",var2)),data=data)
  
  write.csv(as.data.frame(tab),
            paste0(file,"_data.csv"),
            row.names=FALSE)
  ...
  mosaic(
    ...
    labeling_args=list(
      set_varnames=c(var1,var2),
      rot_labels=c(0,0),
      gp_labels=gpar(cex=.6)
    )
  )
...
plot_mosaic(
  diadsP %>% mutate(CoopType=factor(ArrangementScore)),
  "class","CoopType","./plts/Fig4/cls_4.pdf"
)
...
plot_mosaic(
  diadsP %>%
    left_join(
      diadsL %>% select(code,CC_1,CC_2,maxD),
      by=c("code","CC_1","CC_2")
    ),
  "class","maxD","./plts/Fig4/cls_5.pdf"
)


plot_mosaic(
  diadsP,
  "class","has_river_border","./plts/Fig4/cls_s2.pdf"
)