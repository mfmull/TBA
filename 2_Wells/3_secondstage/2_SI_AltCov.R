...
# analysis using an alternative set of matching covariates. The goal is to test
# robustness of the pooled TB vs non-TB effects to a different balancing strategy
# that emphasizes hydro-climatic and contextual variables (e.g., precipitation,
# PET, stress/strain proxies, and distance measures), in addition to location and
# the river-border indicator.
#
# Inputs
...
#       * TB indicator, CC cluster id, and candidate covariates used for matching
#
# Parameters
# - nMin: minimum wells per unit retained
# - cov_match: alternative matching covariate set:
#     lat_c, lon_c, RDS_mainDist, STS, prec_mm, pet_mm, LB_river
...
#    - Enforce complete cases for TB and all matching covariates
#      (filter(if_all(..., ~ !is.na(.)))) so the matching model is well-defined.
...
#     under the alternative matching-covariate specification.
# ------------------------------------------------------------------------------
##############################
#Params
##############################
nMin=20
cov_match =c("lat_c","lon_c",'RDS_mainDist','STS','prec_mm','pet_mm','LB_river')
match='full'
r=1
...
aqf=read.csv('../2_firststage/firststageMain.csv')%>%
 filter(n>nMin)%>%
  ...
  filter(!is.na(TB)) %>%
  filter(if_all(all_of(c(cov_match, "TB")), ~ !is.na(.)))
